...
public class DateUtil {
     ...
     */
    private static class YearMonthDayHourMinuterSecondFormatter implements Formatter<LocalDateTime> {
     ...
     * 匹配格式 yyyy-MM-dd
     */
    private static class YearMonthDayFormatter implements Formatter<LocalDate> {
     ...
     * 处理HH:mm:ss
     */
    private static class HourMinuteSecond implements Formatter<LocalTime> {
     ...
     * @return 解析策略
     */
    private static Formatter FormatterChooser(String pattern) {
        switch (pattern) {
            case YYYY_MM_DD:
    ...
    private static String formatDateByPattern(Object date, String pattern) {
        Formatter Formatter = FormatterChooser(pattern);
    ...
    private static Object parseDateByPattern(String date, String pattern, boolean localDateFlag) {
        ...
        Object parseDate = Formatter.parseDate(date);
        if (localDateFlag) {
            return parseDate;
        }
    ...
    public static int dayOfMouth(Object date) {
        if (date instanceof Date) {
            return ((LocalDateTime) FormatterChooser(YYYY_MM_DD_HH_MM_SS).parseDate(date)).getDayOfMonth();
    ...
    public static int getYear(Object date) {
        if (date instanceof Date) {
            return ((LocalDateTime) FormatterChooser(YYYY_MM_DD_HH_MM_SS).parseDate(date)).getYear();
        } else if (date instanceof LocalDateTime) {
    ...
    public static boolean sameYearMouthDay(Object date01, Object date02) {
        LocalDate localDate01 = (LocalDate) FormatterChooser(YYYY_MM_DD).parseDate(date01);
        LocalDate localDate02 = (LocalDate) FormatterChooser(YYYY_MM_DD).parseDate(date02);
    ...
    public static Date firstOrLastDayInLastMouth(Object date, boolean firstDayFlag) {
        LocalDate localDate;
        if (date instanceof Date) {
            localDate = ((LocalDate) FormatterChooser(YYYY_MM_DD).parseDate(date)).plusMonths(-1);
        } else if (date instanceof LocalDateTime) {
    ...
    public static Date firstOrLastDayInThisMouth(Object date, boolean firstDayFlag) {
        LocalDate localDate;
        if (date instanceof Date) {
            localDate = ((LocalDate) FormatterChooser(YYYY_MM_DD).parseDate(date));
        } else if (date instanceof LocalDateTime) {
